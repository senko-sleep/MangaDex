...
class NHentaiSource extends BaseSource {
  ...
  parseGalleryPages(html, galleryId, detectedExt = null) {
    const pages = [];
    ...
    // Try to detect extension from viewer image in HTML if not provided
    let ext = detectedExt;
    if (!ext) {
      const extMatch = /https:\/\/i\d+\.nhentaimg\.com\/[^"]*\/1\.(jpg|png|gif|webp)/i.exec(html);
    ...
    while ((match = thumbRegex.exec(html)) !== null) {
      ...
      // Use detected extension, or preserve original if detection failed
      const fullUrl = ext 
        ? thumbUrl.replace(/(\d+)t\.(jpg|png|gif|webp)$/, `$1.${ext}`)
        : thumbUrl.replace(/(\d+)t\./, '$1.');
      pages.push({
        index: pageNum,
        url: fullUrl
      });
