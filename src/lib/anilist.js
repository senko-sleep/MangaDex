...
/**
 * Batch fetch covers for multiple manga titles
 * @param {string[]} titles - Array of manga titles
 * @returns {Promise<Map<string, {coverUrl: string|null, bannerUrl: string|null}>>}
 */
export async function batchGetAnilistCovers(titles) {
  ...
  for (let i = 0; i < titles.length; i += batchSize) {
    ...
    const promises = batch.map(async (title) => {
      const result = await getAnilistCover(title);
